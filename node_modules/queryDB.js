/**
 * Created by WQ on 2015/5/14.
 *
 *
 */

var util = require( "../src/js/util.js" );

// 得到一个集合中所有未解决的err
var findAllUnsolvedErr = function ( db, collectionName, callback ) {
    db.collection( collectionName, {strict : true}, function ( err, collection ) {
        if ( err ) {
            callback( "不存在" + collectionName );
        }
        else {
            collection.aggregate(
                [
                    {
                        $match : {
                            solve : false
                        }
                    },
                    {
                        $group : {
                            _id : "$err",
                            count : {
                                $sum : 1
                            }
                        }
                    },
                    {
                        $sort : {
                            count : -1
                        }
                    }
                ],
                function ( err, result ) {
                    callback( err ? err.toString() : null, result );
                }
            );
        }
    } );
};

// 得到集合列表
var getCollectionList = function ( db, callback ) {
    db.collections( function ( err, collections ) {
        if ( err ) {
            callback( err.toString() );
        }
        else {
            var result = [];
            util.forEach( collections, function ( item ) {
                item.s.name != "system.indexes" && result.push( {
                    name : item.s.name
                } )
            } );
            callback( null, result );
        }
    } );
};

// 得到一个项目中的某个错误在今天每小时错误出现的次数
var getTheUnsolvedErrCountInTodayByHours = function ( db, collectionName, errName, callback ) {
    // 构造北京时间今日0点
    var d0 = new Date();
    d0.setMinutes( 0 );
    d0.setSeconds( 0 );
    d0.setHours( 0 );

    // 构造北京当前时间
    var dn = new Date();

    db.collection( collectionName, {strict : true}, function ( err, col ) {
        if ( err ) {
            callback( "不存在" + err );
        }
        else {
            col.aggregate(
                [
                    {
                        $match : {
                            solve : false
                        }
                    },
                    {
                        $project : {
                            hour : {$hour : "$date"},
                            err : 1,
                            date : 1
                        }
                    },
                    {
                        $match : {
                            date : {
                                $gte : d0,
                                $lte : dn
                            },
                            err : errName
                        }
                    },
                    {
                        $group : {
                            _id : {
                                "hour" : "$hour"
                            },
                            count : {
                                $sum : 1
                            }
                        }
                    }
                ],
                function ( err, reslut ) {
                    callback( err, reslut );
                    console.log( err ? err : reslut );
                }
            )
        }
    } );

};


// 得到一个项目在今天每小时错误出现的次数
var getUnsolvedErrCountInTodayByHours = function ( db, collectionName, callback ) {
    // 构造北京时间今日0点
    var d0 = new Date();
    d0.setMinutes( 0 );
    d0.setSeconds( 0 );
    d0.setHours( 0 );

    // 构造北京当前时间
    var dn = new Date();

    db.collection( collectionName, {strict : true}, function ( err, col ) {
        if ( err ) {
            callback( "不存在" + err );
        }
        else {
            col.aggregate(
                [
                    {
                        $match : {
                            solve : false
                        }
                    },
                    {
                        $project : {
                            hour : {$hour : "$date"},
                            err : 1,
                            date : 1
                        }
                    },
                    {
                        $match : {
                            date : {
                                $gte : d0,
                                $lte : dn
                            }
                        }
                    },
                    {
                        $group : {
                            _id : {
                                "hour" : "$hour"
                            },
                            count : {
                                $sum : 1
                            }
                        }
                    }
                ],
                function ( err, reslut ) {
                    callback( err, reslut );
                    console.log( err ? err : reslut );
                }
            )
        }
    } );

};

exports.findAllUnsolvedErr = findAllUnsolvedErr;
exports.getCollectionList = getCollectionList;
exports.getUnsolvedErrCountInTodayByHours = getUnsolvedErrCountInTodayByHours;
exports.getTheUnsolvedErrCountInTodayByHours = getTheUnsolvedErrCountInTodayByHours;